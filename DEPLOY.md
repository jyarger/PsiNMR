...
> **If the wizard forces you to run a connector before continuing:** it shows a
> `docker run cloudflare/cloudflared …` command and waits for it to check in.
> If you run that snippet to get past the wizard, **remove that container
> afterwards** (`docker ps | grep cloudflared`, then `docker rm -f <name>`) —
> it is _not_ on the compose network, cannot resolve `http://psinmr:80`, and
> Cloudflare will load-balance requests onto it, producing intermittent or
> constant **502 Bad Gateway**. The only connector that should run is the
> compose service `psinmr-cloudflared` (start it in step 8; the wizard then
> shows it as connected). (Full detail and the alternatives are in
...
| psinmr.com shows **502 Bad Gateway**                                                            | A connector that can't reach the app is serving requests. Most common: a **stray cloudflared container** left over from running the Cloudflare wizard's `docker run` snippet — it isn't on the compose network, so `http://psinmr:80` doesn't resolve from it, and Cloudflare load-balances requests across **every** connector on the tunnel. `docker ps \| grep -i cloudflared` must show **only** `psinmr-cloudflared`; `docker rm -f` any other. The tunnel's Overview should then list exactly **1 connector**.                                              |
