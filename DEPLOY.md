...
in the wizard's next step, or afterwards from the tunnel's **Routes** tab
(older dashboards call it **Public Hostname**) → add a published application
route:

- **Domain** `psinmr.com` (subdomain blank for the apex; add a second entry for
  `www` if you want it)
- **Service** `HTTP` → `psinmr:80` — this is resolved by the connector
  _inside_ the Docker network, so `psinmr:80` is correct even though it isn't
  a public address.

Without this route the tunnel connects but routes nothing — psinmr.com stays
unreachable even when everything on the box is green.

> **If the wizard forces you to run a connector before continuing:** it shows
> install snippets (Docker: a `docker run cloudflare/cloudflared …` command;
> Debian: `sudo cloudflared service install <token>`) and waits for one to
> check in. If you ran either snippet to get past the wizard, **remove it
> afterwards** — it is _not_ on the compose network, cannot resolve
> `http://psinmr:80`, and Cloudflare load-balances requests across **all**
> connectors on the tunnel, producing intermittent or constant
> **502 Bad Gateway**:
>
> - Docker snippet → `docker ps | grep cloudflared`, then `docker rm -f <name>`
>   (no restart policy, so it does not survive reboots — it may already be gone).
> - Debian snippet → installs a **persistent systemd service that auto-starts
>   at every boot**, so a working site can begin 502ing right after a reboot.
>   Remove: `sudo systemctl disable --now cloudflared && sudo cloudflared service uninstall`.
>
> The only connector that should run is the compose service
> `psinmr-cloudflared` (start it in step 8; the wizard then shows it as
> connected). The tunnel's **Overview** tab must list exactly **1 connector**.
> (Full detail and the alternatives are in
> [Option 1](#option-1--cloudflare-tunnel-recommended) below.)
...
2. **Add the public hostname.** In the tunnel's **Routes** tab (older
   dashboards: **Public Hostname**):
...
| psinmr.com works **intermittently** (200s mixed with 502 "Host Error")                          | Two+ connectors are registered on the tunnel; Cloudflare load-balances across **all** of them, and the extra one can't reach the app. Classic trigger: a host-level **systemd cloudflared** (from the wizard's Debian snippet) auto-starting **after a reboot** — it isn't a container, so `docker ps` looks clean. Check `systemctl status cloudflared` and the tunnel **Overview** connector count (must be **1**). Remove: `sudo systemctl disable --now cloudflared && sudo cloudflared service uninstall`, then verify: `for i in $(seq 1 10); do curl -s -o /dev/null -w '%{http_code} ' https://psinmr.com; done`. |
| psinmr.com shows **502 Bad Gateway** constantly                                                 | A connector that can't reach the app is serving all requests. Most common: a **stray cloudflared container** left over from running the Cloudflare wizard's `docker run` snippet — it isn't on the compose network, so `http://psinmr:80` doesn't resolve from it. `docker ps \| grep -i cloudflared` must show **only** `psinmr-cloudflared`; `docker rm -f` any other. The tunnel's Overview should then list exactly **1 connector**.                                                                                                                                                                                  |
| cloudflared logs: `control stream encountered a failure` / `failed to run the datagram handler` | QUIC (outbound UDP 7844) is being dropped by the VPS network — common on budget providers. The compose file now forces `--protocol http2` (outbound TCP 443): `git pull && docker compose -f docker-compose.prod.yml up -d --force-recreate cloudflared`.                                                                                                                                                                                                                                                                                                                                                                 |
| Tunnel healthy but psinmr.com unreachable                                                       | No **route** on the tunnel (the wizard step is easy to skip if the connector wasn't live yet). Tunnel → **Routes** tab (older dashboards: **Public Hostname**) → add `psinmr.com` → `HTTP` → `psinmr:80`. Also check the tunnel shows a connector as **Connected**.                                                                                                                                                                                                                                                                                                                                                       |
