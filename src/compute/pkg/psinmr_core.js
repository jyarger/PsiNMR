...
export class ContourGrid {
  ...
  contours(levels, timeout_ms) {
    ...
    const ret = wasm.contourgrid_contours(
      this.__wbg_ptr,
      ptr0,
      len0,
      timeout_ms,
    );
    let v2 = getArrayF64FromWasm0(ret[0], ret[1]).slice();
  ...
  constructor(z, rows, cols, xs, ys) {
    ...
    const ret = wasm.contourgrid_new(
      ptr0,
      len0,
      rows,
      cols,
...
if (Symbol.dispose)
  ContourGrid.prototype[Symbol.dispose] = ContourGrid.prototype.free;
...
export function conrec_basic(z, rows, cols, xs, ys, levels, timeout_ms) {
  ...
  let v5 = getArrayF64FromWasm0(ret[0], ret[1]).slice();
  wasm.__wbindgen_free(ret[0], ret[1] * 8, 8);
  return v5;
}
...
export function fft(re, im, inverse) {
  let ptr0 = passArrayF64ToWasm0(re, wasm.__wbindgen_malloc);
  let len0 = WASM_VECTOR_LEN;
  let ptr1 = passArrayF64ToWasm0(im, wasm.__wbindgen_malloc);
...
function __wbg_get_imports() {
  const import0 = {
    __proto__: null,
    __wbg___wbindgen_copy_to_typed_array_4db0cbe2cc60dbee(arg0, arg1, arg2) {
